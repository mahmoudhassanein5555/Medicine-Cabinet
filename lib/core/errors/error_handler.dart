...
class ErrorHandler {
  static Failure handle(dynamic exception) {
    ...
    return Failure((context) => S.of(context).commonError);
  }
  ...
  static Failure _handleFirebaseAuthError(FirebaseAuthException exception) {
    switch (exception.code) {
      case 'invalid-email':
        return Failure((context) => S.of(context).authInvalidEmail);

      case 'user-not-found':
        return Failure((context) => S.of(context).authUserNotFound);

      case 'wrong-password':
        return Failure((context) => S.of(context).authWrongPassword);

      case 'invalid-credential':
        return Failure((context) => S.of(context).authInvalidCredential);

      case 'email-already-in-use':
        return Failure((context) => S.of(context).authEmailAlreadyInUse);

      case 'weak-password':
        return Failure((context) => S.of(context).authWeakPassword);

      case 'too-many-requests':
        return Failure((context) => S.of(context).authTooManyRequests);

      case 'network-request-failed':
        return Failure((context) => S.of(context).authNetworkError);

      case 'user-disabled':
        return Failure((context) => S.of(context).authUserDisabled);

      case 'operation-not-allowed':
        return Failure((context) => S.of(context).authOperationNotAllowed);
      ...
      default:
        return Failure((context) => S.of(context).commonTryAgain);
    }
  }
  ...
  static Failure _handleFirebaseError(FirebaseException exception) {
    switch (exception.code) {
      case 'permission-denied':
        return Failure((context) => S.of(context).authPermissionDenied);

      case 'unavailable':
        return Failure((context) => S.of(context).authNetworkError);

      case 'not-found':
        return Failure((context) => S.of(context).authResourceNotFound);
  ...
  static Failure _handleRemoteError(String message) {
    ...
    if (normalizedMessage.contains('401') ||
      ...
      return Failure((context) => S.of(context).authSessionExpired);
    }
    ...
    if (normalizedMessage.contains('500') ||
      ...
      return Failure((context) => S.of(context).authServerError);
    }
