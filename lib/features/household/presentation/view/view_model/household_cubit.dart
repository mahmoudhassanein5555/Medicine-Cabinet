...
class HouseholdCubit extends Cubit<HouseholdState> {
  ...
  Future<void> createHousehold({
    ...
    result.fold(
      (failure) {
        emit(CreateHouseholdError(failure.failuremessage));
      },
      (household) {
        emit(CreateHouseholdSuccess(household));
      },
    );
  }
  ...
  Future<void> getUserHousehold({required String userId}) async {
    ...
    result.fold(
      (failure) {
        emit(GetHouseholdError(failure.failuremessage));
      },
      (household) {
        emit(GetHouseholdSuccess(household));
      },
    );
  }
  ...
  Future<void> joinHousehold({
    ...
    result.fold(
      (failure) {
        emit(JoinHouseholdError(failure.failuremessage));
      },
      (household) {
